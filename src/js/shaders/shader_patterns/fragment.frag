...
void main() {
    ...
    // vec2 grid = vec2(floor(vUv.x * 10.0) / 10.0, floor(vUv.y * 10.0 + vUv.x * 5.0) / 10.0);
    // float strength = random(grid);
    // gl_FragColor = vec4(vec3(strength), 1.0);
    ...
    // float strength = sin(cnoise(vUv * 10.0) * 20.);
    // gl_FragColor = vec4(vec3(strength), 1.0);

    // float strength = step(0.9, sin(cnoise(vUv * 10.0) * 20.));
    // gl_FragColor = vec4(vec3(strength), 1.0);
